...
from src.common import UnitValues, checks
...
class Units(commands.Cog):
	...
	async def merge_unit(self, ctx, unit: MergeableUnit()):
		...
		embed.add_field(
			name="WARNING",
			value=f"Level up **{unit.display_name}** by consuming **{UnitValues.MERGE_COST}** units?"
		)
		...
		await ctx.bot.db["empires"].update_one(
			...
			{
				"$inc": {
					f"units.{unit.key}.owned": -UnitValues.MERGE_COST,
